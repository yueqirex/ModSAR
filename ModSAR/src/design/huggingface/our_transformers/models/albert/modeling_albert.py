...
class AlbertAttention(nn.Module):
    def __init__(self, config: AlbertConfig):
        ...
        casual_mask = torch.triu(torch.ones(max_len, max_len), diagonal=1)
        min_inf = torch.finfo(casual_mask.dtype).min
        self.casual_mask = casual_mask.masked_fill(casual_mask == 1, min_inf).unsqueeze(0).unsqueeze(0) # (1, 1, max_len, max_len)
    ...
    def forward(
        ...
        # Create a causal mask if this is a uni-directional decoder.
        attention_scores = attention_scores + self.casual_mask.to(attention_scores.device)
